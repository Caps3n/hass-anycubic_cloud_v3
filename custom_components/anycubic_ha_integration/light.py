...
async def async_setup_entry(
    ...
    """Set up Anycubic Cloud light entities."""
    ...
    coordinator.add_entities_for_seen_printers(
        ...
        entity_constructor=AnycubicCloudLight,
        platform=Platform.LIGHT,
        available_descriptors=list(LIGHT_TYPES),
    )


class AnycubicCloudLight(AnycubicCloudEntity, LightEntity):
    """Camera/chamber light for an Anycubic Cloud printer."""
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Turn the light on."""
        await self.coordinator.light_turn_on_event(self._printer_id)

    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn the light off."""
        await self.coordinator.light_turn_off_event(self._printer_id)
